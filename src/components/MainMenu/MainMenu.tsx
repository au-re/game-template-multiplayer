...
export function LoggedInMenu({uid}: {uid: string}) {
    ...
    return <div>
        <label>player name<input readOnly value={uid} /></label>
        <label><input onChange={(e) => setGameIdInput(e.target.value)}/>   
        <button onClick={() => joinGame(gameIdInput)}>join game</button></label>
...
export function LobbyMenu({gameId}: {gameId: string}) {
    ...
    return <div>
        <label>gameId<input readOnly value={gameId} /></label>
